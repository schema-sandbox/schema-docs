...
规模已在上一轮真实材料产物（r5，改动前构建）上核对：8 份成功材料共 50608 个公式区域，其中 `needsVisualFallback: false` 却含 Latin-1 补充区字母的有 80 个，集中在 Math4Kids（71）与 Feynman 讲义（9）。出现的码位以 `Ý`(169)、`Ñ`(115)、`ñ`(64)、`ù`(22)、`Ð`(13)、`ð`(9) 为主，另有少量真实重音 `ö`(6)、`ü`(1)、`é`(1)、`è`(1)。按上下文分，80 个中 31 个是数学形状（如 `xÑx`、`δxÑ0δxÑ0`），其余是重音落在真实单词里（`Mössbauer`、`Müller`、`Ñaña, Peru`），后者是“正文被误判成公式”的另一类缺陷。这里先前把比例记作 31 / 49，划分口径是“文本里有没有能读的单词”；§31 改用逐字符的字体来源重分，结论是 **71 / 9**，后者才是可判定的判据。

因此不能按 Latin-1 区间整体判为损坏：那会把 `Mössbauer` 一类合法文本一起推进视觉回退。可行方向是按码位区分（`Ñ Ý ñ ù ð Ð` 是 TeX 字体码位落到 Latin-1 的典型产物，`ö ü é è` 更像真实重音），但需要逐字体核对码位来源；本轮只记录规模与判据难点，不采用区间启发式。**该核对已在 §31 完成，结论是判据必须取“字符自身的字体”，而不是码位** —— `ô` 既能是 TeX 数学字体里的错位产物，也能是正文字体里的真实重音，只有字体族分得开。
...
## 29. math-deep 的 unmappedGlyphs 十倍变化：已定位为有意的取舍，成本已量化（2026-09-23）

**定位方法（可复用）。** 基线的构建虽不在提交历史里，但它的字节可从 `.ai-doc-exchange/build-current/release/runtime/src/adapters/` 取回：该目录下 `pdfLayoutExtractor.py`(c4139dae)、`pdfLayoutSession.py`(4234b497) 等文件的 sha256 与基线 `runtimeIdentity` 记录逐字节相同。据此可直接 diff 基线代码与当前源码。

**根因。** `preserve_unknown_glyphs`（`src/adapters/pdfLayoutSession.py`）在基线之后新增了一条 `detached_math_symbol` 规则：字体名匹配数学字体且字符属于 `√ ≤ ≥ → ← ↔ ∞ ≲ ≳ ∼ ∈ ∉ × ÷ ± ≡` 时，即使它不是 `(cid:n)` / PUA / U+FFFD，也按 `unmapped_glyph` 处理（注释写明目的是避免 `lower` 变成 `lo√wer`）；同批还加了 0.82 重叠去重守卫。math-deep 因此多出约 1.15 万个单字形区域。

**这是取舍，不是内容丢失，但成本很大。** 同一次 `math-deep.pdf` 的产物对照（基线 `final-cold-20260922-r5` 与当前 `20260922193809-66196` 的 `raw.md`）：

| 指标 | 基线 | 当前 | 变化 |
| --- | --- | --- | --- |
| 行内公式标记 | 2481 | 13924 | +11443 |
| 块级公式标记 | 2452 | 2456 | +4 |
| 图片标记 | 441 | 445 | +4 |
| 表格标记 | 12 | 12 | 0 |
| 正文中残留的 √≤≥→ 等符号 | 11618 | 54 | −11564 |
| markdown 字符数 | 4,029,237 | 5,078,349 | +25% |
| 渲染资源文件数 / 字节 | 5421 / 55,256,064 | 16999 / 77,885,440 | +11578 / +41% |

汇总计数同向：`unmappedGlyphs` 1295 → 12753，`visualFallbackRegions` 4980 → 16557（后者含 `len(glyphs)`），`formulaRegions` 29496 → 28353。

**结论与待决。** 1.15 万个游离符号从正文里被取出，正文变得干净且可编辑；代价是 1.15 万个单字形裁剪、+22.6MB 资源、markdown 臃肿 25%，且这些区域全部计入"需人工复核"。方向与产品既有原则一致（视觉保真不等于可编辑识别），但**规模需要决策**：保持现状、收窄符号集合，还是给每页字形区域设上限。另一个未决子问题：`formulaRegions` 为何同时下降 1143 —— 是否有约千个区域不再被识别为公式，需单独确认。

## 30. §29 两问的收口：−1143 与本批无关；字形规则可留（2026-09-23）

**方法：把两处改动各自单独回退，做单变量对照。** `merge_complex_formula_regions` 的合并条件在基线之后放宽为 `and (not prose_boundary or previous_delimiter_only or current_delimiter_only)`（`src/adapters/pdfLayoutExtractor.py:1108`），同批还加了 `previous_delimiter_only` / `current_delimiter_only` / `displayFragment` 三项判据。把这一行单独还原成 `and not prose_boundary`、其余源码不动，即得到"strict"变体；与当前源码逐页同源对比，差值只可能来自这一行。

**−1143 与本批无关。** 全量 math-deep（2204 页）单变量对照：

| | 当前源码 | strict（仅回退该行） | 该行的影响 |
| --- | --- | --- | --- |
| `formulaRegions` | 28,353 | 28,438 | −85 |
| `unmappedGlyphs` | 12,753 | 12,775 | −22 |
| `visualFallbackRegions` | 16,557 | 16,588 | −31 |
| 行内 / 块级公式标记 | 13,924 / 2,456 | 13,950 / 2,461 | −26 / −5 |

即：合并放宽只解释 1143 中的 **85**（7.4%）。其余约 1058 来自"记录基线"与本批之间的提交距离，而不是本批。此处先前有一处自我纠正：记录基线（`final-r4-full.json`）的 `summary` 里没有 `timingsMs`，也没有 `ocrCandidateCount` / `visualOnlyCandidateRegions`，而这些字段在我从磁盘取回的基线构建里已经存在 —— 说明**记录基线比我取回的基线构建更老**，所以 29496→28353 这段差值跨越多轮提交，本就不该整段归给本批。级联方向自洽：多识别 85 个区域 → 约 22 个符号被公式吸收而不再进入字形采集 → 视觉兜底区域随之减少 31。

**两处改动互相独立，可分别归因。** `formula_regions` 在 `extract_layout_page` 里先于字形采集执行（2434 对 2507），所以字形规则不可能删掉公式区域；字符集合也不相交 —— `EXTENSION_DELIMITERS` 只有括号类 `( ) [ ] \{ \} | \Vert \lfloor \rfloor \langle \rangle`，与符号集合 `√ ≤ ≥ → ← ↔ ∞ …` 无交集。6 页密集页对照（849–854）印证：`formulaRegions` 取回基线构建 57 / strict 57 / 当前 56，`unmappedGlyphs` 98 / 206 / 205 —— 放松合并只动公式区域，字形规则只动字形区域。

**字形规则的成本可精确归因，无需再跑全量。** 对记录运行（`20260922193809-66196`）的 `visual-map.json` 做字形普查：`unmapped_glyph` 区域 12,753 个，其中 11,767 个是**可读的 Unicode 符号**，只有 986 个是真正未映射的 `(cid:n)` / PUA / U+FFFD。既有分支只接受未映射码位，因此那 11,767 个只可能来自新增的 `detached_math_symbol` 分支 —— 这就是该分支的下界成本。构成：`∈`4565、`→`2954、`×`1460、`≤`1186、`≥`1018、`∞`333、`√`92、`∼`68、`±`50、`≡`41；字体 `CMSY10` 11287、`CMEX10` 743、`CMSY8` 499。全书分布均匀而非个别坏页：1763 页增加行内标记、合计 +11553，45 页减少、合计 −110，净 +11443，与记录到的标记差值完全吻合。

**决策：保留该规则。** 对全书 10 页抽样（849–852、100、300、500、1000、1500、2000）逐字形检查邻接：194 个被采集的符号字形中，**0 个**与相邻的非数学字体拉丁字母在 1.5pt 内相接，即不含注释里点明的 `lo√wer` 特征。也就是说它取走的确实是空间上已脱离公式的符号，没有从正文里割字。据此**放弃"收窄符号集合"这一选项** —— 集合里的 `×`、`±` 等易出现在正文的成员，在所有抽样页上都没有从正文被取走，收窄无收益；"每页设上限"同样不适用，因为该现象是全书均匀的，不是坏页放大。代价从此明确记作"这 11.8k 个符号只以裁剪图存在、不可文本检索"，属于接受的取舍。一个未实现的改良方向记录在案：邻接为非正文时保留符号文本（既保检索又不割字），但它需要独立的真实材料全量验证，本轮不引入。

**门禁与回归。** 体积预算随文档增长同步（`src/cli/size-check.js` 与 `src/cli/release-check-part1.js` 的交叉校验字面量必须同时改）。探针产物全部位于 `.ai-doc-exchange/glyph-probe/`（已 gitignore），资产目录用后删除。

## §31 F02 收口：按“字符自身字体”判定，71 个错位区域转源像素兜底（2026-09-23）

**结论：损坏类与真实重音类按字体族严格可分，判据是字符自身的 `fontname`，不是码位区间。** 全书逐字符普查（Math4Kids 691 页、Feynman 2752 页、1512.06808v1 585 页、unit-distance-cot 125 页）显示：Latin-1 字母出现在 `TeX-math*` / `LMMath*` 家族时是生产者 ToUnicode 映射的错位产物，出现在 `LMRoman*` / `Garamond*` / `SFBMR*` / `HFBR*` / `SFRB*` 时是真实重音。两边连字母集合都互不重叠：`Ñ Ý ñ Ð ð ù` 只从数学字体来，`ö é ü á` 只从正文字体来；唯一似有歧义的 `ô` 也分得开 —— 在 `KUKEAY+TeX-matha12` 里是错位（4 处），在 `LMRoman*` 里才是真的。

**证据：Math4Kids 全书普查。** `KUKEAY+TeX-matha12` 319 个（`Ý`130 / `ñ`90 / `Ñ`74 / `ù`16 / `ð`5 / `ô`4）、`LYKCWK+TeX-matha8` 43 个（`Ñ`19 / `Ð`13 / `Ý`11）、`AHQBNY+TeX-mathx10` 1 个；正文字体 `LMRoman12-*` 共 44 个，字母是 `ó ö é á`。arXiv 两篇（Garamond / LMRoman）的 Latin-1 字母 **100% 来自正文字体**，数学字体零命中 —— 所以判据在这些书上不触发，不产生回归。

**机制。** 生产者写的 CMap 系统性错位：数学字体的槽位被映射成 Latin-1 字母，`∂`→`Ñ`、`=`→`“`、`+`→`ˆ`、`(`→`p`、`)`→`q`、`...`→`¨¨¨`，pdfminer 忠实照抄。既有守卫只把 `(cid:n)` / PUA / U+FFFD 当语义损坏，所以这些区域仍被判 `editableMathCandidate: true` —— 也就是把一段自信的错公式交给用户。

**修复。** 新增窄类 `TEX_MATH_FONT`(`tex-math|lmmath|cmmi|cmsy|cmex|msam|msbm|euler|eusm|rsfs`) 与谓词 `math_font_latin1_artifact(chars)`（任一字符的 `text` 是 Latin-1 字母且其 `fontname` 命中窄类即判真），接入三处判定：`formula_regions` 建区时的 `broken`、单字形提升分支的 `semantic_damage`、合并分支的 `semantic_damage`。第二处是真正的漏点 —— 该分支的 `attachment` 会把 `needsVisualFallback` 重置为 `False`，只按文本判损伤时错位区域会从这条路漏回可编辑。命中后区域转源像素裁剪（`needsVisualFallback: true`、`confidence: low`），与 `(cid:n)` 走同一条兜底通道。

**量化（Math4Kids 691 页，单变量 A/B，同一当前源，唯一变量是本修复）。**

| | 修复前 | 修复后 | 说明 |
| --- | --- | --- | --- |
| F02 类区域 / 其中可编辑 | 71 / **18** | 62 / **0** | 18 个自信错公式归零 |
| F02 类区域 / 其中兜底 | 71 / 0 | 62 / **62** | 全部转裁剪 |
| 损坏字母总数 | 296 | 296 | 一个不少 |
| `formulaRegions` | 1035 | 1019 | −16，见下 |
| `visualFallbackRegions` | 9 | 71 | +62，恰等于新增兜底 |
| `failedVisualRegions` | 0 | 0 | 未引入阻塞失败 |
| `unmappedGlyphs` | 9 | 9 | 字形采集未动 |
| `ocrCandidateCount` / `visualOnlyCandidateRegions` | 199 / 183 | 199 / 183 | OCR 账目未动 |
| 落盘资产文件 | 204 | 266 | +62，每个新兜底一张裁剪 |

`formulaRegions` −16 已逐页核对：只有 10 页减少、没有任何一页增加，减少的都是错位碎片被并进同一个兜底区域（例如第 507 页 13→9），即“若干自信错片段”变为“一枚忠实裁剪”，属预期方向。`visualFallbackRegions` 71 = 原有 9 + 新增 62，说明真实重音区域一个都没被误伤。`71→62` 的区域数变化同理是合并所致。

**不做的事。** 不采用 Latin-1 区间规则（会误伤 `Mössbauer`）；不猜测错位偏移去“还原”文本（偏移是生产者的私有约定，还原即臆测）；不动宽类 `MATH_FONT`（另有调用点依赖它把 subset 数学字体当数学字体）。

**同批记录、不在本修复范围：Feynman 9 个是“散文被提升为公式”。** 这 9 个区域（`16-5TheSchrödingerequation`、`Fig.23-10.[CourtesyofDr.R.Mössbauer]` 等页眉/节标题，空格被抹掉）字体是 `SFBMR9` / `HFBR9` / `SFRB1000` —— 命中宽类 `sfbm|sfrb|hfbr`，使 `mathRatio = 1.0`，绕过 `prose_like` 守卫。旧代产物里其中 2 个仍标 `editableMathCandidate: true`。但正文文本仍在 `readable.md` 里（55 处 `Schrödinger`、12 处 `Mössbauer`），所以性质是重复/误提升而非文本丢失，严重度低，列为独立待办：把宽类对“是否数学”的判定与 `prose_like` 的优先级重排。

**同一根因的另一条路径：正文内联数学字形（本轮不动）。** 上述判据只作用于公式区域，而同一错位也落在**正文行**里。Math4Kids 实测：`三个都是白球的概率变得更高了（ 1 Ñ 3 ）`、`（ 1 Ñ 1 ）`、`1 Ñ 0 ）` —— 应为 `1/3`、`1/1`、`1/0`。这些字形夹在句子中间，不进入 `formula_regions`，所以本修复不改变它们：全书输出中的 `Ñ` 由 93 处降到 6 处，剩下的 6 处正是这类内联实例。修法同源（把同一谓词接进 `preserve_unknown_glyphs` 的接受条件，让它们在正文里也走裁剪+标记），但它会把裁剪插进句子中间、使符号不可检索，并且改变全文字形计数；§30 已记录“把字形采集伸进正文需要独立全量验证”，故本轮只记录、不扩大改动。

**回归件。** `test/pdf-formula-layout.test.js` 新增用例覆盖正反两侧：`TeX-matha8`/`TeX-matha12`/`LMMathItalic12` 的 `Ñ Ý ñ ô` 判为损坏，`LMRoman12`/`HFBR9`/`SFBMR9`/`SFRB1000` 的 `ö Å Ñ` 与数学字体里的 `×` 判为正常。探针（`latin1-font-census.py`、`compare-artifact.cjs`）与两轮 691 页产物均在 `.ai-doc-exchange/glyph-probe/`（已 gitignore）。

## §32 侧栏归属的泛化：三条几何判据，正文一字未动（2026-09-23）

**背景与结论。** §22.3 的首要项是「一般侧栏的正文归属」。既有 `isolate_sidebars` 只认一组条件——右半页（`x0 > 页宽*0.6`）+ 字号小于正文 0.85 倍 + 窄——左侧注与正文同号的侧栏都没有任何归属机制。本轮把它泛化为三条几何判据，并证明在真实材料上**严格增量**：整本 Math4Kids（691 页）与 11 个真实窗口的输出与 HEAD 逐字节相同。

**实测漏失（真 PDF 夹具，真值先写后量）。** 页 500×620：正文列 x 40–231（11pt 三行）、左侧注 `cf. 1962`（7.5pt，与正文行同基线）、右侧注 `see chapter 4`（7.5pt）、正文同号侧栏两行（11pt，x 352–430，纵向完全落在正文块之上）。修复前输出：

```
sidebar line one sidebar line two The main column carries the argument.
A second body sentence continues the

see chapter 4

cf. 1962A third body sentence closes it here.
```

即侧栏被并进正文段首，左侧注连分隔都没有、直接粘成 `cf. 1962A third…`（右注因旧的右侧规则而正常）。

**三条判据（`src/adapters/pdfReadingOrder.py`）。** 新增 `body_span`：只取正文同号字符，按 12pt 装订线并成 run，中位字符合在的那个 run 即正文列跨度；新增 `margin_lines`：**整行**判定（字号 < 正文 0.85 倍、整行宽 < 页宽 0.3、整行完全落在正文列之外）；新增「同号侧栏带」：字号 ≥ 正文 0.85 倍、与正文列相隔 ≥12pt、窄（< 页宽 0.35）、纵向与正文块不相交，且只占全页行数的少数。三者都只给首尾字符插 `\n\n`，不删、不重排任何正文词；重复命中的框只 bracket 一次。

**为什么是整行而不是字符 run。** 第一版按字符聚类，把 Campbell 第 51 页页脚 `URRY8743_12_SE_C01_PRF.indd 2 11/6/19 5:16 AM` 从正文列边界处劈成三段（`URRY874` / `3_12_SE_C01_PRF.indd 2 11/6` / `/19 5:16 AM`）。改为整行判定后该页恢复逐字节相同。

**为什么左侧判据挂在正文列而不挂页面比例。** 同一组 arXiv 页里 `Alice`(x449–463)、`Bob`(396–407)、`1 2 n`(140–185) 等小字标签全在正文列跨度 [113.8, 510] **内部**——它们贴着某个符号做上下标，不是页边注。若用「页面左侧 40%」这类比例窗口，它们会被误判（把一句完整句子劈开）。因此左侧判据只认「整行在正文列左缘之外」。

**明确记录、本轮不做的边界。**
1. 正文列**内部**的小字一律不动。几何无法区分「上标／符号下的标签」与「夹在列内的边注」，而旧的右侧比例规则两者都 bracket：在 deep849 它把 `…origin Ω in E,` 与 `we define the map Ω` 从中间劈开（假分段），在 m4k60 / m4k500 / unit-dist 它又把 `j j j`、`2`、`2 3` 正确隔离。两侧都有实例，故本轮**保留旧右侧规则原样**（不让已复核材料的既有行为变化），新判据只做增量。
2. 与正文**纵向交织**的同号侧文不判为侧栏：它与分页双栏的右栏在几何上不可区分，继续交给 `column_flow`。
3. 页内少于 3 行的页面直接跳过：正文列退化成那一行本身，「侧文」无从谈起（Math4Kids 第 628 页居中章名页眉右侧的游离 `˚` 正是被这条挡掉的，挡掉后全书逐字节相同）。

**无回归证据。** 两本整书与 11 个真实窗口的输出与 HEAD 逐字节相同：整本 Math4Kids（691 页）、整本 1512.06808v1（585 页），以及 Campbell 50/51/78、math-deep 84 与 849–854、Math4Kids 60–70 与 500–519、Feynman 1–30 与 9、1512.06808v1 1–20、unit-distance-cot 22；`formulaRegions`/`imageRegions`/`tableRegions`/`unmappedGlyphs`/`visualFallbackRegions`/`renderedImages` 六项计数全部未变。

**回归件。** `test/pdf-page-flow.test.js` 新增两例：一例用真 PDF 端到端断言四组侧文各自成行、正文三句各自整行且不粘连；一例用假 page 直接调 `isolate_sidebars`，断言只有四组侧文的首尾字符被改动，**正文 18 个字符一个都没变**。

## §33 跨页断词与跨页断单元格：真实材料里几乎没有实例，保守拒绝是对的（2026-09-23）

**背景与结论。** §22.3 余下两项是「跨页断词／断单元格」。先量真值再动手，结论是**本轮不改一行代码**：这两类现象在真实材料里近乎不存在，而现有实现对它们的保守拒绝（无词法证据就不合并、只连接完整数据行）正是正确行为。真正在真实材料里高频出现的是另一件事——**页内行尾连字符残留**——它的成因不在断词规则，而在分区/阅读顺序，已单列为下一项。

**跨页断词：1200 个分页边界里 0 例。** 用 pdfplumber 原始文本按物理行扫描（4 本书 × 300 页，页码等页边行先滤除）：1512.06808v1 页内连字符行 52、Math4Kids 5、math-deep 66、Campbell 1528，而「本页最后一行以连字符结尾、下一页第一行以小写字母开头」的边界数**四本全为 0**。放宽到「本页末 4 行 / 下一页首 4 行」以避开页脚与图注的干扰后，Campbell 300 页出现 15 个候选，逐个核对发现首行多为图注或另一栏文字（如 p66→67 的 `hypothesis-` 后面紧跟的是 `idealized process…`，并非该词的续写），不是词续写。管线自身的正文边界（`pdf_page_flow` 去掉页边候选后的 `page.first/last`）在 1512.06808v1 与 math-deep 各 200 页上也没有一条以连字符结尾。即：专业排版本就不在分页处断词。

**跨页断单元格：720 页 184 张表里 0 例。** 判据取「底边越过页高 0.85 的表」紧接「顶边在下一页页高 0.15 以内的表」。6 本书各 120 页：1512.06808v1（111 表，贴底 10、贴顶 2）、Campbell（57 表，贴底 17、贴顶 29）、免疫学（11 表，2/9）、Ecology（5 表，2/1）、math-deep 与 unit-distance-cot 无表；**相邻成对只有 1 例，且那 1 例是 Campbell 第 115 页的章节小结框被读成表，不是被分页截断的数据行**。既有「只连接完整数据行、不猜测断开的单元格」因此没有被真实材料挑战。

**顺手量到的真问题：页内行尾连字符残留。** 同一批 Campbell 60 页（120–179）里，`字母- 空格 小写字母` 形态 25 处、`字母- 换行 小写字母` 9 处，例如 `record- as the tissue heals`、`inflamma- matter what you did tory p`、`hydro- tail phobic`。逐页核对成因有两类，都不是断词规则能解决的：(1) **双栏正文被读成表格**——第 141 页的两栏访谈被识别成 2×2 表，单元格文本用 `; ` 拼接，于是 `re- Dr.`、`cell-`、`biolo-` 全部落在格内；(2) **正文行被图注/图内文字打断**——主栏某行以连字符结尾，阅读顺序里紧跟的却是图注文字，于是 `inflamma-` 与 `tory p` 被别的文字隔开。两者都属于分区与阅读顺序问题，与 §22.3 表头项同源，已作为下一项单列。

**本轮的边界声明。** (1) 本轮不改代码，因此不需要新的回归件；上节 §32 的实测与门禁结论仍然成立。(2) 上面的「0 例」是对这 8 本材料的陈述，不是对所有 PDF 的断言：结论是这两个形态在真实排版里罕见、不足以支撑为它们增加规则，而不是「任何 PDF 都不会出现」。(3) 现有 `reflowPdfParagraphs` 的词法证据判据（拼合词在文内作为独立词出现过才合并）保留原样，未被合并的 `ambiguous_hyphen` 仍作为候选记录，不删除、不改写字符。

**证据位置。** 探针脚本与结果保留在 `.ai-doc-exchange/crosspage-probe/`（`within-page.mjs`、`boundaries.mjs`、`split-cells.mjs`、`report.json`），均为可重跑的小脚本，不含大体积中间产物。

## §34 分栏切分线按区分选：台阶状栏边界不再整页一条线（2026-09-23）

**背景与结论。** §33 把「页内行尾连字符残留」的成因归到分区与阅读顺序。沿这条线索核对真实材料，在 `column_flow` 上找到两条独立缺陷：装订线阈值写死 18pt，以及**一页只允许一条切分线**。两条都改，Campbell 第 97 页上半页的交错消失。

**缺陷一：18pt 阈值恰好压在正文行距上。** Campbell 第 97 页在 `column_flow` 实际保留的 4135 个字符上量（页内 195 个字符落在 63 个图/表区域内被剔除，中位字符高 9.1pt，故带高 `band_height = 18.2`）：`y=464.2` 那一行正文止于 `x=228.9`、窄图注起于 `x=246.2`，通道 **17.3pt**，低于候选端 `right-left >= 18` 与支撑端 `gap < 18`——`cut ± 9` 只是同一阈值的等价写法，18/2=9 > 17.3/2。该处因此不生成切分线，页面级胜出的 `cut=199.0` 把正文与图注归进同一区，按行读成 `break hydroLess ice reduces gen bonds` 式交错。改为具名常量 `COLUMN_GUTTER = 12`（`COLUMN_GUTTER_CLEAR = 6`）统一三处。注意 18.2 正是该页的带高，即旧阈值等于要求通道宽过一整行行距；新阈值取 12，让半通道（6）能容进 17.3pt 的图注装订线。`body_span` 里那个同值的 `GUTTER = 12` 是**另一个概念**（把字符并成 run 的粘连距离），未合并。

**缺陷二：一页一条切分线，台阶状栏边界无解。** 第 97 页上半页要 `x≈302.2` 的线（正文│宽图注，`y=269.2/282.2` 处通道 55.5pt、49.4pt），下半页要 `x≈199` 的线（正文│窄图注，17.3pt）——单条直线无法同时满足。胜出方案 `cut=199.0`（`supportingRows` 14）分区为 `[(-39, 292.2, None), (292.2, 672.2, 199.0), (672.2, 744, None)]`，下半页修好、上半页因整行正文被切开全成跨越行而 `cut=None`。新增 `_refine_zones`：胜出方案留下的整宽区向**其他候选切分线自己的分区**借线——要求该线在该区内有 ≥3 条证据带、借用范围高 ≥3 倍带高，取证据最多者；借到的线只覆盖它自己的证据范围，两端仍留整宽区。借来的分区**自带该线的跨越行屏障**，所以不会把一整行正文劈成左右两半。此后该页分区变为 `[(-39, 292.2, 302.2), (292.2, 672.2, 199.0), (672.2, 744, None)]`，`readingOrder.strategy` 由 `column_major` 变 `segmented_columns`，上半页正文恢复连贯；`flow["top"]/["bottom"]` 一并按新分区重算（它们只在 `zones` 缺失的旧式回退里被读，但会随 `evidence` 写进公开 visualMap，故不留陈旧值）。

**无回归证据。** 9 个真实窗口（`measure.mjs` 的同一组）：ecology、arxiv、math-deep、Math4Kids、unit-distance-cot、Feynman **六个窗口与改前逐字节相同**；Campbell 40–259 连字符残留 30 → 28；immunology 60–179 的整窗哈希虽变，但逐页哈希 120 页里**只有第 71 页变化**。逐页核对确认改动都是改善：第 71 页由「左栏右栏逐词交织」（`from ponents of innate immune`）变为先整栏左、再整栏右；Campbell 220 页里 12 页变化（54、70、76、87、97、118、162、183、201、217、222、225）：第 97 页见上段；第 70、76、87、118、183、201、222 页的句子由交织恢复连贯（第 70、183 页残留各减 1）；其余 54、162、217、225 页是把同一段文字换一条切分线重读、仍有两股交织，属等价改写——**没有任何一页的残留计数变差**。

**回归件。** `test/pdf-page-flow.test.js` 新增一例，构造台阶状栏边界（上半 正文 40–244 / 图注 300–420，下半 正文 40–180 / 图注 200–280），直接调 `column_flow` 断言分区为 `[[0, 157, 271.5], [157, 600, 191.5]]`、策略 `segmented_columns`；关掉 `_refine_zones` 同一输入给出 `column_major` 与 `(0, 157, None)`，即本例只由本轮改动解释。该文件 16 例全过。

**明确记录、本轮不做的边界。** (1) 连字符本身仍未处理：`rejoin_wrapped_prose_lines` 无条件丢掉行尾 `-`，故第 97 页出现 `hydrogenbonded`；Python 侧的页内连接器没有词表可用，JS 侧 `reflowPdfParagraphs` 的「拼合词需在文内出现过」判据没有搬过来，留作下一项。(2) 借用只在胜出方案**留白**的区里发生，不推翻胜出方案自己的分区。(3) 残留计数只看连字符形态，第 97 页与免疫学第 71 页的改善对它是**不可见**的（两页计数都未变），所以此轮结论主要靠逐页哈希与逐页目视复核，不靠该计数。
