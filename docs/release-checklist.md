...
All commands should pass before a public v0.1.4 tag. Use `npm run release-readiness -- --report-only --out <readiness.json>` when an internal handoff needs an auditable JSON readiness report without changing release state. The default `npm run release-readiness` command must pass before tagging.
...
- `npm run size-check` stays under the enforced lightweight budgets: 0 runtime dependencies, at most 1 dev dependency, 1.75MB runtime bytes, 2.8MB source/test/docs bytes, 125KB largest source file, 125KB largest runtime file, 100KB public browser module file, 132 runtime files, 263 checked source files, and 58,500 checked source lines.
...
- `npm run fixture-check -- --strict` passes before a public v0.1.4 tag.
...
- `npm run release-readiness` reports `readyForPublicTag: true` and exits successfully before a public v0.1.4 tag.
...
- `npm run desktop-verification-fill -- --record <partial-record.json> --diagnostics-pass --first-workflow-pass --workspace-picker-pass --file-picker-pass --spreadsheet-preview-evidence <spreadsheet-preview.json> --workspace-images-evidence <workspace-images.json> --save-picker-evidence <save-picker.json> --segmented-html-export-evidence <segmented-html-export.json> --result-pass --tester <name> --windows-version <windows-version> --node-version <node-version> --webview2-present yes --out <filled-record.json>` fills explicitly confirmed visible UI evidence and required environment metadata into the partial record after the tester has completed each manual Desktop UI check. `--visible-ui-pass` covers only the earlier general steps; it cannot satisfy the four structured regression evidence objects.
...
- A user can complete workspace selection, import, conversion/export, exchange package creation, package read-back, and receiver/trust report generation from the desktop path before public v0.1.4.
