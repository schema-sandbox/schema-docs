...
| `.pdf` | Core + optional developer enhancement | Built-in text extraction, readability checks, formula-encoding diagnostics, and source-preserving visual maps. | The Office workflow never requires a model runtime: damaged formulas remain source-linked images. **Scientific refinement** and **Marker** are developer-only optional local reconstruction tools for specialist mathematical PDFs. **Poppler** renders mapped regions. **Tesseract OCR + Poppler** handles scanned PDFs. |
...
- **Editable scientific Markdown**: The default Office path keeps trustworthy formulas editable and preserves uncertain formulas as source-linked images; it never guesses damaged formula semantics. Developer tools may retry with `scientific` or `marker` when a separately installed local model runtime is available. These optional modes can take hours on multi-thousand-page books.
