...
### Admitted desktop dependency: `tauri-plugin-dialog`

- **Why required:** v0.1.4 uses `tauri-plugin-dialog` 2.7.2 for file, folder, Markdown-open, and export-save dialogs that are owned by the Schema Docs window. This replaces the detached PowerShell/WinForms picker process and restores reliable modality, focus, and cancellation behavior.
- **Main-path status:** it is compiled into the Windows desktop shell, but its API runs only after an explicit user picker action. It is not used by the browser UI or local Node server. The transitive `tauri-plugin-fs` 2.5.1 dependency comes only through the dialog plugin and is not separately initialized or exposed to frontend code.
- **Size and native-binding risk:** the Cargo graph also adds `rfd` 0.16.0 and platform dialog bindings. It adds no npm runtime dependency or separately shipped helper executable. Rebuilt EXE, MSI, NSIS, and portable-ZIP byte sizes and SHA-256 values are regenerated by the Windows release preparation step before handoff.
- **Data access:** the operating-system dialog can display local file/folder names and returns only the path the user explicitly selects. Schema Docs then applies extension, existence, workspace-boundary, canonical-path, and one-use export-authorization checks before reading or writing.
- **Network access:** none is requested or used by this integration.
- **Replacement plan:** prefer a future Tauri core window-owned dialog API if it provides the same ownership and cancellation guarantees. A replacement must retain the current command boundary and tests; detached PowerShell/WinForms dialogs are not an accepted fallback.
