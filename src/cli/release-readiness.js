...
function buildNextActions({ releaseCheck, fixtureStrict }) {
  ...
  if (releaseCheck.result?.automaticChecksPassed !== true) {
    actions.push("Fix failing automatic release-check items before preparing a public v0.1.3 tag.");
  }
  ...
  if (fixtureStrict.result?.ok !== true && blockingItems.length === 0) {
    actions.push("Fix fixture-check --strict failures before preparing a public v0.1.3 tag.");
  }
