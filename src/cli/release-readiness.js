...
function buildNextActions({ releaseCheck, fixtureStrict }) {
  ...
  if (releaseCheck.result?.automaticChecksPassed !== true) {
    actions.push("Fix failing automatic release-check items before preparing a public v0.1.4 tag.");
  }
  ...
  if (fixtureStrict.result?.ok !== true && blockingItems.length === 0) {
    actions.push("Fix fixture-check --strict failures before preparing a public v0.1.4 tag.");
  }
  ...
  return {
    ...
    desktopGate: {
      ...
      requiredEvidence: [
        ...
        "real multi-sheet spreadsheet preview",
        "visible PPTX and PDF workspace images",
        "native save picker ownership and cancellation recovery",
        "complete segmented PDF HTML export on disk",
