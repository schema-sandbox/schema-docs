...
const nextActionByFailure = {
  ...
  spreadsheet_preview_not_pass: "Import a real multi-sheet XLSX file and record visibleUi.spreadsheetPreview only after real rows render and switching sheets visibly changes the rows.",
  spreadsheet_real_rows_not_visible: "Confirm real spreadsheet cell values are visibly rendered, not only an import-success notification.",
  spreadsheet_multiple_sheets_not_visible: "Confirm at least two spreadsheet sheet choices are visible.",
  spreadsheet_sheet_switch_not_worked: "Switch between at least two sheets and confirm the visible rows change.",
  workspace_images_not_pass: "Import PPTX and PDF fixtures and visibly inspect at least one workspace image from each format.",
  workspace_images_pptx_natural_width_not_positive: "Inspect a rendered PPTX image and confirm its DOM naturalWidth is greater than zero.",
  workspace_images_pptx_broken_image_present: "Confirm the inspected PPTX workspace image is not broken.",
  workspace_images_pdf_natural_width_not_positive: "Inspect a rendered PDF image and confirm its DOM naturalWidth is greater than zero.",
  workspace_images_pdf_broken_image_present: "Confirm the inspected PDF workspace image is not broken.",
  save_picker_not_pass: "Open the native save picker from the packaged Desktop UI and cancel it after the ownership checks.",
  save_picker_not_visible: "Confirm the native save picker is visibly displayed.",
  save_picker_not_bound_to_main_window: "Confirm the native save picker is modal to and owned by the Schema Docs main window.",
  save_picker_cancel_not_restored: "Cancel the save picker and confirm the main app regains focus and remains responsive.",
  segmented_html_export_not_pass: "Export a segmented PDF from the packaged Desktop UI to a complete HTML file.",
  segmented_html_pdf_not_selected: "Use a PDF whose readable output is segmented for the HTML export check.",
  segmented_html_segments_incomplete: "Confirm the exported HTML includes content from every PDF segment, including the last segment.",
  segmented_html_file_not_written: "Confirm the HTML export completes and the selected output file exists on disk.",
  segmented_html_file_empty: "Confirm the exported HTML file on disk is non-empty.",
...
const failures = [
  record.releaseTarget !== "v0.1.4" && fail("release_target_mismatch", record.releaseTarget),
...
if (strict) {
  failures.push(...[
    ...
    visibleUi.spreadsheetPreview?.status !== "pass" && fail("spreadsheet_preview_not_pass", visibleUi.spreadsheetPreview?.status),
    visibleUi.spreadsheetPreview?.realRowsVisible !== true && fail("spreadsheet_real_rows_not_visible", null),
    visibleUi.spreadsheetPreview?.multipleSheetsVisible !== true && fail("spreadsheet_multiple_sheets_not_visible", null),
    visibleUi.spreadsheetPreview?.sheetSwitchWorked !== true && fail("spreadsheet_sheet_switch_not_worked", null),
    visibleUi.workspaceImages?.status !== "pass" && fail("workspace_images_not_pass", visibleUi.workspaceImages?.status),
    visibleUi.workspaceImages?.pptxNaturalWidthPositive !== true && fail("workspace_images_pptx_natural_width_not_positive", null),
    visibleUi.workspaceImages?.pptxBrokenImageAbsent !== true && fail("workspace_images_pptx_broken_image_present", null),
    visibleUi.workspaceImages?.pdfNaturalWidthPositive !== true && fail("workspace_images_pdf_natural_width_not_positive", null),
    visibleUi.workspaceImages?.pdfBrokenImageAbsent !== true && fail("workspace_images_pdf_broken_image_present", null),
    visibleUi.savePicker?.status !== "pass" && fail("save_picker_not_pass", visibleUi.savePicker?.status),
    visibleUi.savePicker?.pickerVisible !== true && fail("save_picker_not_visible", null),
    visibleUi.savePicker?.boundToMainWindow !== true && fail("save_picker_not_bound_to_main_window", null),
    visibleUi.savePicker?.cancelRestoredApp !== true && fail("save_picker_cancel_not_restored", null),
    visibleUi.segmentedHtmlExport?.status !== "pass" && fail("segmented_html_export_not_pass", visibleUi.segmentedHtmlExport?.status),
    visibleUi.segmentedHtmlExport?.segmentedPdfSelected !== true && fail("segmented_html_pdf_not_selected", null),
    visibleUi.segmentedHtmlExport?.allSegmentsIncluded !== true && fail("segmented_html_segments_incomplete", null),
    visibleUi.segmentedHtmlExport?.htmlFileWritten !== true && fail("segmented_html_file_not_written", null),
    visibleUi.segmentedHtmlExport?.htmlFileNonEmpty !== true && fail("segmented_html_file_empty", null),
