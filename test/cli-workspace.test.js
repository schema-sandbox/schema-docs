...
test("CLI commands for inbox, timeline, versions, settings, and real samples work", async () => {
  ...
  const converted = await execFileAsync(process.execPath, [cliPath, "convert-text", workspace, docId], { cwd: projectRoot });
  const markdownPath = path.relative(workspace, JSON.parse(converted.stdout).output.outputMarkdownPath).split(path.sep).join("/");
  const { stdout: versionsList } = await execFileAsync(process.execPath, [cliPath, "versions", workspace, "list", markdownPath], { cwd: projectRoot });
  ...
  assert.ok(JSON.parse(allVersionsList).some((version) => version.path === markdownPath));
  const { stdout: versionsListShorthand } = await execFileAsync(process.execPath, [cliPath, "versions", workspace, markdownPath], { cwd: projectRoot });
  assert.deepEqual(JSON.parse(versionsListShorthand), versions);
  const { stdout: promoteRes } = await execFileAsync(process.execPath, [cliPath, "versions", workspace, "promote", markdownPath, versions[0].id], { cwd: projectRoot });
