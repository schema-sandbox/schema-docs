...
test("release-artifacts reports build artifact checksums when present", async () => {
  ...
  assert.equal(result.releaseTarget, "v0.1.4");
  ...
  assertArtifact(result, (artifact) => artifact.path.endsWith("schema-docs_0.1.4_x64-portable.zip"));
...
test("release-index writes tester handoff artifact index", async () => {
  ...
  assert.equal(result.targetVersion, "v0.1.4");
...
test("release-check validates desktop runtime bridge and current size budget", async () => {
  ...
  const expectedFields = {
    ...
    windows_release_checksums_current: {
      ...
      requiredAssets: [
        "schema-docs_0.1.4_x64_en-US.msi",
        "schema-docs_0.1.4_x64-setup.exe",
        "schema-docs_0.1.4_x64-portable.zip"
      ],
